...
class LaVITforPigeonDPO(nn.Module):
    def __init__(
        self,
        model_path="",
        model_dtype="bf16",
        ...
        **kwargs
        ...
        if training:
            ...
            self.llama_model.load_adapter(lora_weights, adapter_name="reference")
            self.llama_model.set_adapter("policy")
        ...
        if self.mask_type == 'mutual':
            self.mask_generator = MutualMaskGenerator(
                emb_dim=4096,  # model.config.hidden_size,  # 4096
                num_heads=num_heads,
    ...
    def prepare_input_for_lavit(
        ...
        if self.mask_type == "random":
            all_image_tokens, labels, masked_labels = self.tokenize_image_with_random_mask(image_list, mask_ratio, add_special, with_mask, inference)
            ...
            all_image_tokens, labels, masked_labels = self.tokenize_image_with_mutual_mask(image_list, mask_ratio, hist_mask_ratio, add_special, with_mask, inference)
        else:
            all_image_tokens, labels, masked_labels = self.tokenize_image_wo_mask(image_list, add_special, inference) 
        ...
        if not inference:
            ...
            if self.mode == "dpo" or self.mode == "ipo":
                ...
                merged_rejected_input_tokens, merged_rejected_attention_masks, merged_rejected_labels = None, None, None

            return merged_chosen_input_tokens, merged_chosen_attention_masks, merged_chosen_labels, merged_rejected_input_tokens, merged_rejected_attention_masks, merged_rejected_labels, tgt_embs
        else:
            merged_input_tokens, merged_attention_masks, _ = self.merge_batch_inputs(
                all_image_tokens, text_input_ids, text_attn_mask,
                inference, padding=self.llama_tokenizer.padding_side,
            )
    ...
    def concatenated_inputs(self, chosen: torch.Tensor, rejected: torch.Tensor, max_length: int, pad_value: Union[int, float]):
        chosen_pad_size = list(chosen.shape)
        chosen_pad_size[-1] = max_length - chosen.size(-1)
        rejected_pad_size = list(rejected.shape)
        rejected_pad_size[-1] = max_length - rejected.size(-1)

        padding_side = self.llama_tokenizer.padding_side
        if padding_side == "left":
            chosen = torch.cat([pad_value * torch.ones(*chosen_pad_size, dtype=chosen.dtype, device=chosen.device), chosen], dim=-1)
            rejected = torch.cat([pad_value * torch.ones(*rejected_pad_size, dtype=rejected.dtype, device=rejected.device), rejected], dim=-1)
        else:
            chosen = torch.cat([chosen, pad_value * torch.ones(*chosen_pad_size, dtype=chosen.dtype, device=chosen.device)], dim=-1)
            rejected = torch.cat([rejected, pad_value * torch.ones(*rejected_pad_size, dtype=rejected.dtype, device=rejected.device)], dim=-1)
    ...
    def forward(self, batch, hist_mask_ratio, add_special, use_cache=None):
        mask_ratio = random.uniform(0, 1)
        chosen_input_ids, chosen_attention_mask, chosen_labels, \
            rejected_input_ids, rejected_attention_mask, rejected_labels, \
            tgt_embs = self.prepare_input_for_lavit(
        ...
        if self.mode == "dpo" or self.mode == "ipo":
            ...
            input_ids = self.concatenated_inputs(chosen_input_ids, rejected_input_ids, max_length, pad_value=self.llama_tokenizer.pad_token_id)
            attention_mask = self.concatenated_inputs(chosen_attention_mask, rejected_attention_mask, max_length, pad_value=0.0)
            labels = self.concatenated_inputs(chosen_labels, rejected_labels, max_length, pad_value=-100)
            tgt_embs = torch.cat((tgt_embs, tgt_embs), dim=0)
        else:
        ...
        all_logps, size_completion = self.get_batch_logps(all_logits, labels)
        nll_loss = self.cross_entropy_loss(all_logits[:len_chosen], labels[:len_chosen])
