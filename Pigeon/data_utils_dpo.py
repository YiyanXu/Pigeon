...
class DPODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        chosen_tokens = self.data["chosen_tokens"][idx]
        rejected_tokens = self.data["rejected_tokens"][idx]
    ...
    def select(self, indices):
        ...
        return DPODataset(selected_data)
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self.return_tensors == "pt":
            for key in features.keys():
                if key == "chosen_tokens" or key == "rejected_tokens":
                    # keep these output tokens as list
                    continue
...
def process_dpo_data(
    ...
    for uid in data:
        for genre in data[uid]:
            ...
            for hist_seq in hist_seqs:
                iid = hist_seq[-1]
                chosen_tokens = preference_data[uid][iid][genre]["chosen"]
                rejected_tokens = preference_data[uid][iid][genre]["rejected"]
                ...
                all_chosen_tokens.append(chosen_tokens)
                all_rejected_tokens.append(rejected_tokens)
    ...
    return {"text_input_ids": text_input_ids, "text_attn_mask": text_attn_mask,
            "chosen_tokens": all_chosen_tokens, "rejected_tokens": all_rejected_tokens,
